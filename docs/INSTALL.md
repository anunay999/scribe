...
(Replace `anunay-aatipamula` with whatever owner you publish under.)
...
```bash
ln -s ~/dev/llm-wiki-claude/plugins/llm-wiki ~/.claude/plugins/local/llm-wiki
```
...
Look for `wiki-bootstrap`, `wiki-ingest`, `wiki-query`, `wiki-lint`, `wiki-update`. If they're missing, run `/marketplace reload` and try again.

## Uninstall

```text
/uninstall llm-wiki
/marketplace remove llm-wiki-claude
```
...
**`/wiki-bootstrap` says it can't write to `~/Documents/`.** macOS sandboxing on the Claude Code binary may block reads but allow writes. The skill detects this and proceeds; check with `test -d ~/Documents/obsidian/claude/`. If writes truly fail, pick a vault root outside `~/Documents/` (`~/notes/`, `~/dev/wiki/`, etc.).
...
**`wiki-ingest` keeps re-fetching the same URL.** Check `sources/<slug>.md` exists. If the slug differs from what the skill picked, rename the file or pass a custom slug: `/wiki-ingest <url> --slug=my-slug` (planned for v0.2; today, paste with the slug embedded).
