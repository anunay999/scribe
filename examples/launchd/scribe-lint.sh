#!/bin/zsh
# scribe-lint — runs a wiki health check headlessly via `claude -p` over the
# Obsidian vault. Invoked by ~/Library/LaunchAgents/dev.scribe.lint.plist on a
# twice-daily schedule (09:00 and 21:00 local by default).
#
...
# in the headless session (claude -p doesn't always recognize slash commands).
#
# Output is appended to $HOME/Library/Logs/scribe/lint.log with a timestamp.
#
# Override the vault path at runtime:  SCRIBE_VAULT=/path/to/vault scribe-lint.sh
...
# Ensure ~/.local/bin and homebrew bin are on PATH so launchd finds claude / git.
